...
# Si se quiere hacer con los ficheros scaled habría que cambiar el *-metrics -> *-scaled-metrics
for f in os.listdir("."):
	if fnmatch(f, "*-metrics_by_class.csv") \
	and (f != "Complete_results-metrics_by_class.csv") \
	and (f != "Complete_results-scaled-metrics_by_class.csv"):
		print(f" # File {f}")
		df_ = pd.read_csv(f)
