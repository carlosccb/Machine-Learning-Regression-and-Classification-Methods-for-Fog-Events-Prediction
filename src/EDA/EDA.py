...
####################################################################################
# Sacar las gráficas para ver la relación entre las variables utilizadas
####################################################################################

#Ver la relación entre las variables del problema en el que se predice la duración de la niebla y el mínimo valor
sns.scatterplot(x=df["dB"][:,-2],y=df["dB"][:,-1])

data=[]
vals = np.unique(df["dB"][:,-1])
for i in vals:
	pos = (i == df["dB"][:,-1])
	data.append(np.mean(df["dB"][pos,-2]))
sns.scatterplot(x=vals, y=data)


#Ajustar modelo con transformación log-log y plotear
#Esto es con la media de los datos 
...
for i in range(1,5):
	...
	sns.scatterplot(x=np.log(vals),y=np.log(data))
	sns.lineplot(np.arange(0,6,0.1),trendpoly(np.arange(0,6,0.1)))
	print(" Degree: " + str(i) + " - RMSE: " + str(rmse(trendpoly(np.log(vals)), np.log(data))))
	plt.clf()
...
# Intentar hacer la estimación del polinómio con regresión lineal con regularización
#para que los puntos extremos no influyan tanto
#Hacer con regularización
...
# ####################################################################################
# # Intentar ajustar un modelo a los datos muestrales
# ####################################################################################

# #No fufa | Source: https://stackoverflow.com/a/3433503
# trend = np.polyfit(x=df["dB"][:,-2],y=np.log(df["dB"][:,-1]), deg=4)
# trendpoly = np.poly1d(trend)
# plt.plot(df["dB"][:,-2], df["dB"][:,-1],'o')
# plt.plot(df["dB"][:,-2], trendpoly(df["dB"][:,-2]), 'o')

# # --------------------------------------------------

# # Sacado de https://stackoverflow.com/a/37616966
# %matplotlib inline
