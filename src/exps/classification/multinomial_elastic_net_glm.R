...
LOG_PATH  <- str_c(ROOT_PATH, "Niebla/log_files/alg_comp/classification/nominal/")
PRED_PATH <- str_c(ROOT_PATH, "Niebla/log_files/predictions/classification/nominal/")
...
get_metrics <- function(y.true, y.pred, verbose=FALSE) {
  ...
  # Esto hace que pete la función y como no se usa de todas maneras no hace falta arreglarlo
...
if(is.numeric(dataset_num)) {
  ...
  print("Using dataset all DATASETS")
}
...
for(DATASET in DATASETS) {
  ...
  for(i in 0:10) {
    ...
    start_time <- Sys.time()
    # En teoría habría que hacer esto, pero los resultados sacados no lo hicieron
    set.seed(0) # Quitado para garantizar reproducibilidad
    ...
    y.pred <- predict(fitted_model, s=fitted_model$lambda.min, newx=x.ts, type = "class")
    y.pred <- as.integer(y.pred)
    y.prob <- predict(fitted_model, s=fitted_model$lambda.min, newx=x.ts)
  
    #TODO: Guardar las predicciones para sacar las métricas en un script a parte (¿en python?)
    write.table(y.pred, str_c(PRED_PATH,"GLM_elnet-Nominal-",DATASET,"-alpha=",alpha,"-pred_class.csv"), row.names=F, col.names=F, sep=",") # Non-Scaled data
    write.table(y.prob, str_c(PRED_PATH,"GLM_elnet-Nominal-",DATASET,"-alpha=",alpha,"-pred_probs.csv"), row.names=F, col.names=F, sep=",") # Non-Scaled data

    # Quitado por que no se usa y hace que pete por la función get_metrics al hacer transpose
    # 
  ...
  # Quitado por que no se usa y hace que pete por la función get_metrics al hacer transpose
  #   
  # # write.csv(metrics_cls, str_c(LOG_PATH,"GLM_elnet-",DATASET,"-scaled-metrics_by_class.csv"), row.names=F)
  # # write.csv(metrics_cls, str_c(LOG_PATH,"GLM_elnet-",DATASET,"-scaled-metrics_by_class.csv"), row.names=F)
  # write.csv(metrics_avg, str_c(LOG_PATH,"GLM_elnet-Nominal",DATASET,"-metrics_avg.csv"),      row.names=F)
  # write.csv(metrics_avg, str_c(LOG_PATH,"GLM_elnet-Nominal",DATASET,"-metrics_avg.csv"),      row.names=F)
}

if(is.numeric(dataset_num)) {
  # Guardar con el nombre del dataset ejecutado
  write.csv(exec_time_df, str_c(LOG_PATH, "elapsed_times-Nominal-", DATASETS, ".csv"), row.names=F)
} else {
  # Guardar con nombre genérico
  write.csv(exec_time_df, str_c(LOG_PATH, "elapsed_times-Nominal-ALL.csv"), row.names=F)
}
